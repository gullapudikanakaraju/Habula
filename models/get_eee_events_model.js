module.exports = function(data, request, response){
	...
	console.log('in get_eee_events_model.js');
	console.log(data.documents_skip);
	eee_events_model.find().where('verified').equals(true).sort({_id : -1}).skip(data.documents_skip).limit(5).exec(function(err, result){
		if(err)
		{
