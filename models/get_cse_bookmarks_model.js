module.exports = function(data, request, response){
	var cse_bookmarks_model = require('../schemas/cse_events_schema.js');
	console.log('in get_cse_bookmarks_model.js');
	/*
	...
	*/
	...
	cse_bookmarks_model.find().where('bookmarks.bookmarked_by._id').equals(data.user_id).sort({'bookmarks.bookmarked_by.bookmarked_at' : -1}).skip(data.documents_skip).limit(5).exec(function(err, result){
		if(err)
		{
			console.log('error occurred while fetching data regarding cse events'+err);
			response.send(JSON.stringify({success: false}));
